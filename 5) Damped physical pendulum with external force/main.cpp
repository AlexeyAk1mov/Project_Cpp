...
int main(int atgc, char *argv[]){
    ...
    long double dt = atof(argv[3]);
    double gamma = atof(argv[4]);
    double w0 = atof(argv[5]);
	...
	data_t(dt, N, t, x, v, gamma, w0, w, F);

        remove("numbers1.txt");
        std::ofstream out1(path1);
        out1.open("numbers1.txt", std::ios::app);
	...
	t.resize(0);
        ...
        predict_correct_data_t(dt, N, t, x, v, gamma, w0, w, F);

        remove("numbers2.txt");
        std::ofstream out2(path2);
        out2.open("numbers2.txt", std::ios::app);
